...
abstract class RetryContextUtil
{
    ...
    static void recoverConnectables(final MuleContext muleContext, final RetryContext retryContext)
    {
        ...
        {
            LOGGER.debug("Recovering connectables for: " + retryContextDescription);
        }

        final Collection<AbstractConnector> connectors = muleContext.getRegistry().lookupObjects(
    ...
    /**
     * This method is the Achile's heel of the whole policy: it is based on the fact
     * that the retry context description will contain the string representation of
     * the recovered connector which will be of the expected form. There must be a
     * better way to do this.
     */
    private static boolean hasConnectorBeenReconnected(final String retryContextDescription,
                                                       final AbstractConnector connector)
    {

        return retryContextDescription.contains("name=" + connector.getName());
    }
